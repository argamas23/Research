...
def main():
    ...
    if args.delete:
        from delete_source import delete_book
    ...
    results_root = os.path.join(pipeline_dir, "results")
    ...
    # extract_text.py usage: python extract_text.py input.pdf output.txt
    run_step(
        "step_1_txt",
        f"Step 1: Translating '{book_name}' to TXT",
        ["python3", os.path.join(pipeline_dir, "extract_text.py"), pdf_path, txt_path],
        state_path,
        state,
        [txt_path],
    )
    ...
    # extract_entities.py usage: --input <file> --output <file>
    run_step(
        "step_3_ner",
        "Step 3: Running NER (extract_entities.py)",
        [
            "python3", os.path.join(pipeline_dir, "extract_entities.py"),
            "--input", txt_path,
    ...
    # 4. Topics
    unique_topics_path = os.path.join(pipeline_dir, "unique_topics.txt")
    # Usage: --ner_file <file> --output_file <file>
    run_step(
        "step_4_topics",
        "Step 4: Extracting Topics (deduplicate_topics.py)",
        [
            "python3", os.path.join(pipeline_dir, "deduplicate_topics.py"),
    ...
    selected_topics_path = os.path.join(pipeline_dir, "selected_topics.txt")
    
    # Usage: --corpus_file --ner_file --selected_topics --all_topics --output_file
    run_step(
        "step_5_cooccurrence",
        "Step 5: Entity Co-occurrence (extract_cooccurrences.py)",
        [
            "python3", os.path.join(pipeline_dir, "extract_cooccurrences.py"),
    ...
    run_step(
        "step_6_relations",
        "Step 6: Relation Extraction (extract_relations.py)",
        [
            "python3", os.path.join(pipeline_dir, "extract_relations.py"),
            "--corpus_file", txt_path, # Use the single book text file
    ...
    # 7. Correct Entity Types using TextRazor NER
    run_step(
        "step_7_correct_entity_types",
        "Step 7: Correcting Entity Types via NER (classify_entities.py)",
        ["python3", os.path.join(pipeline_dir, "classify_entities.py")],
        state_path,
        state,
    )

    # 8. Rebuild HTML Visualization from all pipeline/results graph CSVs
    run_step(
        "step_8_rebuild_graph",
        "Step 8: Rebuilding HTML Visualization (rebuild_graph.py)",
        ["python3", os.path.join(pipeline_dir, "rebuild_graph.py")],
        state_path,
        state,
    )
    ...
    run_step(
        ...
        "Step 9: Auditing Salt Recall (audit_salt_recall.py)",
        ["python3", os.path.join(pipeline_dir, "audit_salt_recall.py")],
        state_path,
        state,
        [os.path.join(pipeline_dir, "outputs", "salt_recall_audit.csv")],
    )
