...
HTML_PATHS = [
    PIPELINE_DIR / "network_visualization.html",
    Path(OUTPUT_DIR) / "network_visualization.html",
]
CYTOSCAPE_HTML_PATHS = [
    PIPELINE_DIR / "network_cytoscape.html",
...
def main() -> None:
    ...
    css_src = PIPELINE_DIR / "network_visualization.css"
    if css_src.exists():
        shutil.copyfile(css_src, Path(OUTPUT_DIR) / "network_visualization.css")
    cytoscape_src = PIPELINE_DIR / "lib" / "cytoscape-3.30.4" / "cytoscape.min.js"
    ...
    vis_src = PIPELINE_DIR / "lib" / "vis-9.1.2" / "vis-network.min.js"
    if vis_src.exists():
